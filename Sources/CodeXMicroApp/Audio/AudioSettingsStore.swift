...
final class AudioSettingsStore: ObservableObject {
        ...
        meterTimer = Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { [weak self] _ in
            MainActor.assumeIsolated {
                ...
                if self.isRunning && SupportedRemoteID.allCases.contains(where: {
                    self.configuration.inputs[AudioMixConfiguration.remoteUID($0)] != nil && !RemoteMappingStore.shared.connected.contains($0)
                }) {
    ...
    func refresh() {
        ...
        if isRunning {
            let used = Set(configuration.inputs.keys).union(configuration.outputs.keys)
    ...
    func quickSelect(_ uid: String, input: Bool, enabled: Bool) {
        ...
        if resume && !configuration.inputs.isEmpty && !configuration.outputs.isEmpty {
            Task {
    ...
    func select(_ uid: String, input: Bool, enabled: Bool) {
        if isRunning || isStarting { stopMix(); message = "混音设备已修改，点击启动混音应用新选择。" }
    ...
    func update(_ channel: AudioMixChannel, uid: String, input: Bool) {
        ...
        for (uid, output) in outputs { output.state.configure(inputs: configuration.inputs, output: configuration.outputs[uid] ?? AudioMixChannel()) }
        save()
    }
    ...
    func startMix() async {
        ...
        if let error = configuration.validationError(availableInputs: Set(inputs.map(\.uid)).union(remoteUIDs), availableOutputs: Set(outputDevices.map(\.uid)), feedbackSensitiveUIDs: Set(devices.filter(\.isLoopback).map(\.uid))) { message = error; return }
        for remote in SupportedRemoteID.allCases where configuration.inputs[AudioMixConfiguration.remoteUID(remote)] != nil {
        ...
        let generation = startGeneration
        if configuration.inputs.keys.contains(where: { !$0.hasPrefix("remote:") }) {
            let permission = AVCaptureDevice.authorizationStatus(for: .audio)
            let granted: Bool
            if permission == .notDetermined { granted = await AVCaptureDevice.requestAccess(for: .audio) }
            else { granted = permission == .authorized }
    ...
    func receiveRemote(_ samples: [Int16], rate: Int, remote: SupportedRemoteID) -> Bool {
        ...
        guard configuration.inputs[uid] != nil else { return true }
