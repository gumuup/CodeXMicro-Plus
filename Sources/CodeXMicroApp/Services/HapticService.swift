...
final class HapticService {
    ...
    func press(strength: HapticStrength, soundEnabled: Bool) {
        ...
        if soundEnabled {
            keySound?.stop()
            keySound?.playbackDeviceIdentifier = SystemAudioDevices.text(
                SystemAudioDevices.defaultDevice(kAudioHardwarePropertyDefaultSystemOutputDevice), kAudioDevicePropertyDeviceUID
            )
    ...
    func radialConfirmation(strength: HapticStrength, soundEnabled: Bool) {
        if strength != .off { NSHapticFeedbackManager.defaultPerformer.perform(.generic, performanceTime: .now) }
        if soundEnabled {
            keySound?.stop()
            keySound?.playbackDeviceIdentifier = SystemAudioDevices.text(
                SystemAudioDevices.defaultDevice(kAudioHardwarePropertyDefaultSystemOutputDevice), kAudioDevicePropertyDeviceUID
