...
- `CodeXMicro++-2.0.1-universal.dmg`：macOS 通用安装包，支持 Apple Silicon 与 Intel Mac。
...
- 右键面板上的任意功能键，可以录制、改绑或清除对应的全局快捷键。
- 推理旋钮左右半区和四向摇杆可分别配置快捷键。
- 支持 A1–A6、FAST、同意、拒绝、新任务、语音、标注切换和 Codex 状态等共 19 个动作。
- 录制时按 `Esc` 取消，按 `Delete` 清除；重复或被系统占用的组合会给出提示。
- 快捷键配置保存在本机，不会上传。
...
| 拒绝 | `⌃]` | 拒绝或取消当前请求 |
| 新任务 | `⌃N` | 创建新的 Codex 任务 |
| 左下圆形触控键 | `⌃H` | 显示或隐藏面板按键标注 |
| 麦克风 | `⌃⇧D` | 开始或结束 Codex 语音听写 |
| CodeX 状态键 | `⌃C` | Codex 运行时切换周剩余/累计消耗；未运行时打开 Codex |
| 推理旋钮左半区 | `⌃-` | 降低一级推理强度 |
| 推理旋钮右半区 | `⌃=` | 提高一级推理强度 |

`⌃` 表示 Control，`⇧` 表示 Shift。所有默认组合都可以通过右键对应面板控件重新设置。
