...
- `CodeXMicro++-2.7.0-universal.dmg`：macOS 通用安装包，支持 Apple Silicon 与 Intel Mac。
...
- 面板支持 A1–A6、FAST、同意、拒绝、新任务、语音、标注切换和 Codex 状态等 19 个动作；另有快速启动和悬浮位置切换，共 21 个可配置功能。
...
`⌃` 表示 Control，`⌥` 表示 Option，`⇧` 表示 Shift。19 个面板功能可以右键对应控件设置；快速启动与悬浮位置切换可在设置页单独配置。
...
- 菜单栏可显示、隐藏或退出悬浮面板；全局默认 `⌥Space`，菜单内也保留 `⌥⌘M` 切换方式。
