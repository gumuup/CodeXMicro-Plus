...
- **X6 语音键持续开麦**：语音键改为按一下开启、再按一下关闭，松开实体键不会结束收音；兼容会在按键松开后主动停止首段语音的固件，并对开麦失败执行有限重试。
- **混音配置更稳健**：已保存但暂时断开的输入或输出不再阻止其他可用设备启动混音，原配置会完整保留，设备恢复后无需重新选择。
- **明确的混音控制**：修改参与混音的设备后不再自动重启收音，统一由用户点击“启动混音”；启动失败会显示明确提示，界面只统计当前可用通道。
- **硬件电量可靠性**：遥控器与 MX Master 3S 定期刷新电量；暂时缺失或越界的读数不会清空最近一次有效结果，MX Master 3S 会在需要时重新发现电量功能。
- **硬件动作即时响应**：快捷键、应用、系统应用和网址类硬件映射直接执行，不再排在 Codex 界面自动化队列后等待。
- **X6 原生事件过滤提速**：HID 事件先到时立即匹配并拦截对应原生键，只有 Quartz 事件先到时才使用短暂识别窗口。
- **版本升级**：应用版本、构建号、Codex 客户端标识、安装说明和通用 DMG 统一更新为 5.1.0（构建 510）。
...
- **剪贴板历史侧栏**：默认按 `⌃V` 从当前屏幕右侧呼出近期记录，可搜索文字和图片，选择后会恢复内容并粘贴回原应用；保留时长、清空历史与快捷键可在设置中管理。
- **本地隐私与持久化**：剪贴板历史仅保存在本机 Application Support，默认保留 7 天、最多 100 项，并可随时停用或清空。
- **设置迁移**：剪贴板启用状态和保留时长会随应用设置备份与恢复；实际复制内容不会被打包进设置备份。
- **版本升级**：应用版本统一更新为 4.0.0（构建 400）。

## 3.2.0 功能更新

- **Ulanzi Dial 双向映射**：底层识别旋钮的相对输入，顺时针与逆时针可分别录制成两个独立功能键，并把每个刻度转换成完整的按下/松开脉冲。
- **常见复合控制器适配**：扩展支持鼠标、摇杆、游戏手柄、键盘、数字键盘、多轴控制器及 Consumer Control 顶层集合，可识别设备里的按钮、媒体键、滚轮与旋钮。
- **厂商自定义控件兼容**：支持复合设备中的厂商自定义相对控件和二值按键，同时过滤绝对轴、状态字段等非按键输入，避免误录或功能持续按下。
- **稳定的设备身份匹配**：映射会保存用途页、用途、方向以及设备顶层用途；同一设备的按钮与旋钮方向可以准确区分，设备显示名称变化不会导致绑定失效。
- **旧配置无损升级**：3.1.0 及更早版本保存的 HID 宏键无需重新设置即可继续使用；普通鼠标左右键仍保留安全录制保护。
- **媒体键可读标注**：播放/暂停、上一首、下一首、静音及音量按键会显示对应名称，便于在自定义按键和轮盘中确认映射。

## 3.1.0 功能更新

- **鼠标按键映射**：在快捷键录制界面可直接按鼠标中键、侧键或带修饰键的左右键，将它们映射到快速启动、轮盘及面板功能。
- **原始 HID 宏键监听**：支持识别部分高阶鼠标不会转换成普通点击事件的独立宏键，并按设备、型号和 HID 用途保存映射。
- **可靠的按住/松开语义**：鼠标与宏键会去除重复按下，并完整处理松开事件，语音等功能继续保持按住开始、松开结束。
- **安全录制保护**：不允许无修饰键的鼠标左键或右键被完全接管，避免用户失去正常点击能力；鼠标产生的绑定也不会被误当作键盘快捷键执行。
- **光标旁快速启动**：使用快速启动快捷键显示悬浮面板时，面板会居中出现在当前光标附近，并自动限制在当前屏幕可用区域内。
- **十组轮盘预设**：每个全局或应用轮盘可保存 10 组独立组合并即时切换；旧版单组配置会自动迁移到第 1 组。
- **轮盘复制粘贴**：支持右键或 `⌘C` / `⌘V` 复制完整轮盘操作，在不同位置和预设组合间快速复用。
- **扩展图标库**：轮盘图标支持分类 Emoji、扩展 SF Symbols 和本地图片，并提供中英文关键词及模糊搜索。
- **数据管理与备份**：新增数据页，可更换保存路径、备份/恢复、完整性检查、修复重建及清理未使用的自定义素材。
- **开机自启动**：通用设置中可直接开启或关闭登录时自动启动，并显示 macOS 登录项审批状态。
...
- 面板支持 A1–A6、FAST、同意、拒绝、新任务、语音、标注切换和 Codex 状态等 19 个动作；另有快速启动、轮盘、剪贴板侧栏和悬浮位置切换，共 23 个可配置功能。
...
- 鼠标中键、侧键、原始 HID 宏键、Ulanzi Dial 顺/逆时针，以及复合控制器中的二值按键、媒体键和相对旋钮也可直接录制；左右键必须搭配 Control、Option、Shift 或 Command，避免影响正常点击。部分设备映射的显示名称会包含设备名称、控件用途或方向。
...
- 每个应用或全局轮盘支持 10 组预设组合，每组可配置 1–12 项操作；建议保留 6–10 项以形成稳定的方向记忆，操作顺序即顺时针排列顺序。
- 轮盘操作支持右键复制/粘贴；设置窗口未在编辑文字时也可使用 `⌘C` / `⌘V`，完整复制名称、图标、动作及单项快捷键。
...
- 图标选择器支持分类 Emoji、扩展 macOS SF Symbols 与本地图片；中英文关键词、拼音片段和轻微拼写错误均可参与模糊搜索。
...
- 不包含遥测、广告或第三方分析 SDK。
- 不上传任务内容、按键记录或账户用量。
- 物理按键映射收到的键盘和鼠标按键事件只在内存中比较按键与修饰键，不记录未命中的输入内容。
- 应用设置、轮盘配置与自定义图标默认保存在本机 Application Support；用户可在“设置 → 数据”中自行备份、恢复或更换保存目录。
- 不会把本机开发签名私钥写入项目目录。
